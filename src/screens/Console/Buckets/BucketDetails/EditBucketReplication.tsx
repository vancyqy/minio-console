    ...
    <Fragment>
      ...
      <PageLayout>
        ...
        onSubmit={(e: React.FormEvent<HTMLFormElement>) => {
          ...
          help={
          <Fragment>
                    <Box sx={{ paddingTop: "10px" }}>{t("For each write operation to the bucket, MinIO checks all configured replication rules for the bucket and applies the matching rule with highest configured priority.")}



            </Box>
                    <Box sx={{ paddingTop: "10px" }}>{t("MinIO supports enabling replication of existing objects in a bucket.")}


            </Box>
                    <Box sx={{ paddingTop: "10px" }}>{t("MinIO does not enable existing object replication by default. Objects created before replication was configured or while replication is disabled are not synchronized to the target deployment unless replication of existing objects is enabled.")}
