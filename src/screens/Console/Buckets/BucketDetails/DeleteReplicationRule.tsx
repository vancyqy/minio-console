...
const DeleteReplicationRule = ({
  ...
  return (
    <ConfirmDialog
    title={
    deleteSelectedRules ? t("Delete Selected Replication Rules") : t("Delete Replication Rule")}
    ...
    <Fragment>
          {deleteSelectedRules ?
      <Fragment>{t("Are you sure you want to remove the selected replication rules for bucket")}

        <b>{selectedBucket}</b>?<br />
              <br />{t("To continue please type")}
        <b>{t("Yes, I am sure")}</b>{t("in the box.")}
        <Grid item xs={12}>
                <InputBox
          id="retype-tenant"
          name="retype-tenant"
      ...
      <Fragment>{t("Are you sure you want to delete replication rule")}
        {" "}
              <b>{ruleToDelete}</b>?
            </Fragment>}
