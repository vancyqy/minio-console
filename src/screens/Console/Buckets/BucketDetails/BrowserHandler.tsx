...
const BrowserHandler = () => {
  ...
  const internalPaths =
    ...
    // pathSegment.length === 2 ?  Base64.encode(decodeURIComponent(pathSegment[1])) : "";
  ...
  const initWSRequest = useCallback(
    (path: string) => {
      ...
      console.log("...request ws", payloadData);
  ...
  const pathLoad = useCallback(
    (forceLoad: boolean = false) => {
      ...
      console.log("searchPath,simplePath,bucketName,selectedBucket,forceLoad: ",searchPath,simplePath,bucketName,selectedBucket,forceLoad);
