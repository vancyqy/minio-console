...
const BucketNamingRules = ({ errorList }: {errorList: boolean[];}) => {
  ...
  t("Bucket names must be between 3 (min) and 63 (max) characters long.");
  const characterRuleText =
  t("Bucket names can consist only of lowercase letters, numbers, dots (.), and hyphens (-).");
  const periodRuleText =
  t("Bucket names must not contain two adjacent periods, or a period adjacent to a hyphen.");
  const ipRuleText =
  t("Bucket names must not be formatted as an IP address (for example, 192.168.5.4).");
  const prefixRuleText = t("Bucket names must not start with the prefix xn--.");
  const suffixRuleText =
  t("Bucket names must not end with the suffix -s3alias. This suffix is reserved for access point alias names.");
