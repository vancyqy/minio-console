    ...
    if (dataObject.version_id === undefined) {
      ...
      if (distributedSetup) {
        ...
        listObjects(bucketName, {
          ...
          // prefix: Base64.encode(dataObject.name || ""),
      ...
      onClose={() => {
        ...
        <Fragment>
          ...
          sx={{
            ...
            placement="right"
            tooltip={
            <span>{t("You can reset your session by logging out and logging back in to the web UI.")}

              <br /> <br />{t("You can increase the maximum configuration time by setting the MINIO_STS_DURATION environment variable on all your nodes.")}


              <br /> <br />{t("You can use")}
              <b>{t("mc share")}</b>{t("as an alternative to this UI, where the session length does not limit the URL validity.")}

            </span>}>

              
                <span>{t("The following URL lets you share this object without requiring a login.")}

                <br />{t("The URL expires automatically at the earlier of your configured time (")}

                {niceTimeFromSeconds(maxShareLinkExpTimeVal)}{t(") or the expiration of your current web session.")}
