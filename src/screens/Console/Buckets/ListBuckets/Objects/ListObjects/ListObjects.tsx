...
import { BucketQuota,accessPolicyMap } from "api/consoleApi";
...
getSessionGrantsWildCard } from
"../../UploadPermissionUtils";
import {
makeid,
removeTrace,
storeCallForObjectWithID,
storeFormDataWithID } from
"../../../../ObjectBrowser/transferManager";
import {
cancelObjectInList,
completeObject,
failObject,
...
setBucketInfo } from
"../../../BucketDetails/bucketDetailsSlice";
import {
downloadSelected,
openAnonymousAccess,
openPreview,
openShare } from
"../../../../ObjectBrowser/objectBrowserThunks";
...
const ListObjects = () => {
      ...
      if (allowedFiles.length) {
        ...
        if (allowedFiles.length !== acceptedFiles.length) {
          ...
          setErrorSnackMessage({
            ...
            [IAM_SCOPES.S3_PUT_OBJECT, IAM_SCOPES.S3_PUT_ACTIONS], t("upload objects to this location"))
  ...
  const onClosePanel = (forceRefresh: boolean) => {
    ...
    if (detailsOpen && selectedInternalPaths !== null) {
      ...
      // `/browser/${encodeURIComponent(bucketName)}/${Base64.encode(URLItem)}`);
  ...
  const multiActionButtons = [
  {
    ...
    [IAM_SCOPES.S3_GET_OBJECT, IAM_SCOPES.S3_GET_ACTIONS], t("download objects from this bucket"))
  ...
  },
  {
    ...
    icon: <ShareIcon />,
    tooltip: canShareFile ? t("Share Selected File") : t("Sharing unavailable")
  },
  {
    action: () => {
    ...
    icon: <PreviewIcon />,
    tooltip: canPreviewFile ? t("Preview Selected File") : t("Preview unavailable")
  },
  {
    action: () => {
    ...
    icon: <AccessRuleIcon />,
    tooltip:
    selectedObjects.length === 1 && selectedObjects[0].endsWith("/") ? t("Set Anonymous Access to this Folder") : t("Anonymous Access unavailable")
  ...
  },
  {
    ...
    tooltip: canDelete ? t("Delete Selected Files") :

    permissionTooltipHelper(
    [IAM_SCOPES.S3_DELETE_OBJECT], t("delete objects in this bucket"))
      ...
      <PageLayout variant={"full"}>
        ...
        <Box withBorders sx={{ padding: "0 5px" }}>
            ...
            "& .detailsSpacer": {
              ...
              "@media (max-width: 600px)": {
                marginRight: 0
                  ...
                  <span className={"detailsSpacer"}>{t("Created on:")}
              ...
              <strong>
                ...
                ""}
                  ...
                  <span className={"detailsSpacer"}>{t("Access:")}

              {/* <strong>{bucketInfo?.access || ""}</strong> */}
              <strong>{accessPolicyMap.find(item=>item.value === bucketInfo?.access) ? accessPolicyMap.filter(item=>item.value === bucketInfo?.access)?.map((it:any)=> t(it.label))[0] : ''}</strong>
                  </span>
                  {bucketInfo &&
            <Fragment>
          ...
          <Fragment>
            ...
            <TooltipWrapper
            tooltip={
            canRewind ? t("Rewind Bucket") :

            permissionTooltipHelper(
            [
            IAM_SCOPES.S3_GET_OBJECT,
            IAM_SCOPES.S3_GET_ACTIONS,
            IAM_SCOPES.S3_GET_BUCKET_VERSIONING], t("apply rewind in this bucket"))}>
