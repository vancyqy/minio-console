...
const SiteReplication = () => {
  ...
  return (
    <Fragment>
      <PageHeaderWrapper label={t("Site Replication")} actions={<HelpMenu />} />
      <PageLayout>
        <SectionTitle
        separator={!!hasSites}
        sx={{ marginBottom: 15 }}
        ...
        <Grid container>
            ...
            help={
            <Fragment>{t("This feature allows multiple independent MinIO sites (or clusters) that are using the same external IDentity Provider (IDP) to be configured as replicas.")}
        ...
        iconComponent={<ClustersIcon />}
        help={
        <Fragment>{t("This feature allows multiple independent MinIO sites (or clusters) that are using the same external IDentity Provider (IDP) to be configured as replicas. In this situation the set of replica sites are referred to as peer sites or just sites.")}




          <br />
                <br />{t("Initially, only one of the sites added for replication may have data. After site-replication is successfully configured, this data is replicated to the other (initially empty) sites. Subsequently, objects may be written to any of the sites, and they will be replicated to all other sites.")}
          ...
          <br />
                <br />{t("All sites must have the same deployment credentials (i.e. MINIO_ROOT_USER, MINIO_ROOT_PASSWORD).")}


          <br />
                <br />{t("All sites must be using the same external IDP(s) if any.")}

          <br />
                <br />{t("For SSE-S3 or SSE-KMS encryption via KMS, all sites must have access to a central KMS deployment server.")}
        ...
        confirmationContent={
        <Fragment>{t("Are you sure you want to remove all the replication sites?.")}
