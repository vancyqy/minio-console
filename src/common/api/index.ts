...
export class API {
  invoke(method: string, url: string, data?: object, headers?: RequestHeaders) {
    ...
    if (targetURL[0] === "/") {
      ...
      targetURL = window.globalConfig.baseUrlXhr ?  window.globalConfig.baseUrlXhr+targetURL :  targetURL.slice(1);

    }else{
      targetURL = window.globalConfig.baseUrlXhr ?  window.globalConfig.baseUrlXhr+'/'+targetURL : targetURL ;
